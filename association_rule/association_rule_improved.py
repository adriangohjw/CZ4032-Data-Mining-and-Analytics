...
from data_processor import load_dataset, filter_df_columns, append_col_name_to_dataframe, get_binning_references, bin_dataframe, get_frequent_itemsets, get_association_rules, unmap_classifiers_with_binning_refereces
...
# Optimiing the dataframe to reduce the memory footprint
#########################

# Display the memory (In MB) used by the dataframe BEFORE optimization
print("BEFORE ------------------------------")
print(get_df_memory_size(df_with_appended_column_names))

# Start the "stopwatch" to measure the time taken to optimize the dataframe
start = time.process_time()

# Generate the optimized dataframe and reference dictionary from the current dataframe
df_optimized, reference_dict = get_optimized_df_and_dict(df_with_appended_column_names)

# Stop the "stopwatch" to measure the time taken to optimize the dataframe
elapsed_time = time.process_time() - start

# Print out the time taken to optimize the dataframe in seconds
print(">>>>> Time to determine optimize dataframe: " + str(elapsed_time) + "secs")

# Print out the size of the optimized dataframe in MB
print()
print("AFTER ------------------------------")
...
# Mapped the optimized dataframe back to its original data for ease of understanding the rules
...
#########################
# Remaining steps are the same as the original implementation
# It will be ran to show that the end results are the same
#########################
